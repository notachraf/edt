...
public class AppController {
    ...
    public void genereEDT(){
        DonneesEDT donneesEDT = new DonneesEDT();
        donneesEDT.setListeModules(mApp.getListModule());
        donneesEDT.setListeProfesseurs(mApp.getListeProfs());
        donneesEDT.setListePromotions(mApp.getListePromos());
        donneesEDT.setListeSalles(mApp.getListeSalles());
        mGenerateurEDT= new GenerateurEDT(0.9, 10000.0, new ArrayList<>(), null, donneesEDT);
    ...
    public void afficheEDT(){
        mEDTCalendarPane.getCalendarSources().clear();
        mEDTCalendarPane.showMonthPage();
        ...
        for( Map.Entry<Integer, List<Evenement>> even : map.entrySet()){
            for (Evenement e : even.getValue()) {
                ...
                int years = LocalDate.parse(e.getGroupe().getPromotion().getLocalDate()).getYear();
                int mois = LocalDate.parse(e.getGroupe().getPromotion().getLocalDate()).getMonth().getValue();
                int  jours = LocalDate.parse(e.getGroupe().getPromotion().getLocalDate()).getDayOfMonth();
                entry.setInterval(LocalDate.of(years, mois, jours));
